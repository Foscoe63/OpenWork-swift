...
public final class SessionEvents: @unchecked Sendable {
    ...
    public func progress(token: String, kind: String, title: String? = nil, message: String? = nil, percentage: Int? = nil) {
        lock.withLock {
            switch kind {
            case "begin":
    ...
    public var progressState: (active: Int, lastActivity: Date) {
        lock.withLock { (activeProgress.count, lastActivity) }
    }

    public func publish(path: String, version: Int?, items: [[String: Any]]) {
        lock.withLock { diagnostics[path] = PublishedDiagnostics(version: version, items: items, received: Date()) }
    }

    public func published(path: String) -> PublishedDiagnostics? {
        lock.withLock { diagnostics[path] }
    }

    public func record(_ changes: [(String, FileChange)]) {
        lock.withLock {
    ...
    public func takeFileChanges() -> [String: FileChange] {
        lock.withLock {
...
public actor LanguageServerSession {
    ...
    public nonisolated let connection: LSPConnection
    public nonisolated let events: SessionEvents
    ...
    public static let maxOpenDocuments = 40
    ...
    public static func start(_ resolution: LanguageServerCatalog.Resolution, initializeTimeout: TimeInterval = 120) async throws -> LanguageServerSession {
    ...
    public func open(_ path: String) throws -> (uri: String, text: String) {
        flushFileChanges()
    ...
    public static func answerServerRequest(method: String, params: Any?, root: String, events: SessionEvents) -> Result<Any, LSPConnection.ServerRequestError> {
        switch method {
    ...
    public static func handleNotification(method: String, params: [String: Any]?, events: SessionEvents) {
        switch method {
        case "$/progress":
