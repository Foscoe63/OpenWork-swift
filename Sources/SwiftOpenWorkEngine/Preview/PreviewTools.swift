...
public enum PreviewTools {

    public static let names: Set<String> = ["preview_start", "preview_check", "preview_logs", "preview_stop"]

    /// Tools that only read. Allowed in plan mode, never need approval.
    public static let readOnly: Set<String> = ["preview_check", "preview_logs"]

    // MARK: preview_start

    @MainActor
    public static func start(arguments: [String: Any], workspace: Workspace, settings: AppSettings, startTime: CFAbsoluteTime) async -> ToolExecutionResult {
    ...
    @MainActor
    public static func check(arguments: [String: Any], workspace: Workspace, startTime: CFAbsoluteTime) async -> ToolExecutionResult {
        let sessions = PreviewSessions.shared
    ...
    @MainActor
    public static func logs(arguments: [String: Any], startTime: CFAbsoluteTime) -> ToolExecutionResult {
    ...
    @MainActor
    public static func stop(startTime: CFAbsoluteTime) -> ToolExecutionResult {
        let manager = DevServerManager.shared
    ...
    private static func revealPaneIfWatched() {
        ...
        EngineHosting.host?.revealPreviewIfWatched()
    }

    /// The approval prompt for `preview_start`, or nil when it runs nothing.
    public static func approvalReason(argumentsJson: String) -> String? {
