...
public final class MockLLMService: LLMProviderClient, Sendable {
    ...
    public func streamChat(
        ...
        let workspacePath = extractWorkspacePath(from: lastUserMessage)
            ?? "/Volumes/WorkSpaces/SwiftOpenWork"
    ...
    private func executeMorningBrief(
        ...
        (generated by SwiftOpenWork offline fallback when a live model server was unavailable).
    ...
    /// Avoid matching path segments like `SwiftOpenWork` as a code request.
    private func looksLikeCodeRequest(_ lower: String) -> Bool {
        ...
        if lower.contains(" swift ") || lower.hasPrefix("swift ") || lower.hasSuffix(" swift") {
            return !lower.contains("/volumes/") && !lower.contains("swiftopenwork")
        }
        return false
    }
