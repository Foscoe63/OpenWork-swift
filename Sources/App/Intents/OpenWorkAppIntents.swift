...
struct AskOpenWorkIntent: AppIntent {
    static var title: LocalizedStringResource = "Ask OpenWork"
    static var description = IntentDescription(
        "Run a prompt through your current OpenWork agent and return its reply.",
        categoryName: "Agents"
    )
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog {
        ...
        guard !trimmed.isEmpty else {
            throw OpenWorkIntentError.emptyPrompt
        }
...
struct RunAutomationIntent: AppIntent {
    ...
    static var description = IntentDescription(
        "Run one of your OpenWork automations and return what it produced.",
        categoryName: "Agents"
    )
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog {
        guard let stored = AppState.shared.automations.first(where: { $0.id == automation.id }) else {
            ...
            throw OpenWorkIntentError.automationNotFound(automation.name)
        }
        guard stored.isEnabled else {
            throw OpenWorkIntentError.automationDisabled(stored.name)
        }
...
enum OpenWorkIntentError: Swift.Error, CustomLocalizedStringResourceConvertible {
    case emptyPrompt
    ...
    var localizedStringResource: LocalizedStringResource {
        ...
        case .automationNotFound(let name):
            return "The automation “\(name)” no longer exists in OpenWork."
        case .automationDisabled(let name):
            return "The automation “\(name)” is turned off in OpenWork."
        }
    }
}
...
struct OpenWorkShortcuts: AppShortcutsProvider {
    static var appShortcuts: [AppShortcut] {
        AppShortcut(
            intent: AskOpenWorkIntent(),
