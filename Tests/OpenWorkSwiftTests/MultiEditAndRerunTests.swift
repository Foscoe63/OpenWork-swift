...
final class RerunFailingTestsTests: XCTestCase {
    ...
    func testExtractsXCTestIdentities() {
        let output = """
        Test Case '-[OpenWorkSwiftTests.MultiEditTests testEditsApplyInOrder]' started.
        /repo/Tests/MultiEditTests.swift:12: error: -[OpenWorkSwiftTests.MultiEditTests testEditsApplyInOrder] : XCTAssertEqual failed
        Test Case '-[OpenWorkSwiftTests.MultiEditTests testEditsApplyInOrder]' failed (0.01 seconds).
        """
        XCTAssertEqual(
            BuildDiagnostics.failedTests(in: output),
            [.init(suite: "OpenWorkSwiftTests.MultiEditTests", name: "testEditsApplyInOrder")],
    ...
    func testUnknownRunnerIsNotNarrowed() {
        ...
        XCTAssertNil(BuildDiagnostics.rerunCommand(baseCommand: "cargo test", failures: [.init(name: "a")]))
    }
