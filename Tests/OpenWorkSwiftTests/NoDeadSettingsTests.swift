...
final class NoDeadSettingsTests: XCTestCase {
    ...
    private static let knownDead: Set<String> = [
        // Two whole settings pages for a feature that does not exist. Deleting them is a product
        // decision, so they keep honest UI instead; see HANDOFF.
        "cloudSyncEnabled", "cloudControlPlaneUrl", "cloudAccountEmail", "cloudOrganizationName",
        // A switch for an update feed that does not exist. Its control is disabled and says so.
        "autoCheckForUpdates",
    ...
    private static var sourceRoot: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()   // OpenWorkSwiftTests
    ...
    func testEverySettingIsReachableFromTheUI() throws {
        ...
        let notUserFacing: Set<String> = [
            ...
            "settingsSchemaVersion",
            // Its row exists and is deliberately *disabled*, bound to `.constant(false)` with a
            // subtitle saying this build has no update feed — so the field is referenced nowhere,
            // which is the honest state for a switch that would otherwise promise a check that
            // cannot happen.
            "autoCheckForUpdates"
        ]
